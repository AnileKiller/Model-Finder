...
class ApplyBeautyUseCase {
    ...
    private fun applyUnderEyeReduction(
        ...
        // 1. Local Illumination Map (Fixes directional lighting & asymmetry)
        val blurRadius = (w * 0.04f).toInt().coerceIn(15, 60)
        val blurPixels = gaussianBlur(pixels, w, h, blurRadius)

        // 2. Constrain to skin and combine masks
        ...
        // 3. Feather the mask edge gently
        ...
        for (y in 0 until h) {
            for (x in 0 until w) {
                ...
                val lum = luma(p) 
                val localLum = luma(blurP)

                // 4. Adaptive Shadow Lifting via Local Contrast
                val shadowDepth = maxOf(0f, localLum - lum)
                val shadowFactor = smoothstep(3f, 35f, shadowDepth) 
                ...
                // 5. Dynamic Color Rebalancing
                val rMinusB = r - b
                val blueDominance = smoothstep(-30f, 0f, -rMinusB.toFloat())
                val redBoost = (adaptiveLift * blueDominance * 0.25f).toInt()

                var nr = r + adaptiveLift + redBoost
                var ng = g + adaptiveLift
                var nb = b + adaptiveLift

                // 6. The "Ashy" Protector
                if (nr < ng - 10) ng = nr + 10 
                if (nr < nb - 10) nb = nr + 10 

                nr = nr.coerceIn(0, 255)
                ng = ng.coerceIn(0, 255)
                nb = nb.coerceIn(0, 255)
    ...
    companion object {
        ...
        private const val MAX_UNDER_EYE_LIFT        = 180f // Increased significantly for higher max strength
